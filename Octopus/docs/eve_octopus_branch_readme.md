...
rm -f Octopus/ros2_ws/src/octopus_camera_transform/octopus_camera_transform/*.backup.* 2>/dev/null || true
...
## Terminal 1 — Pi / Pixhawk bridge / MicroXRCEAgent
...
## Terminal 4 — quick input check before Octopus
...
```text
/fmu/out/vehicle_odometry: around 70–100 Hz
```
...
```text
/camera/image_raw/compressed: around 7–15 Hz
```
...
```text
/detector_node/confirmed: around 1 Hz
```

Only continue when all three inputs are alive.

---

## Terminal 5 — start Octopus indoor-static stack

Before pressing enter, make sure the drone is physically aligned:

```text
Drone fixed in the middle.
Drone/camera front points toward demo-room front.
Robot/home station behind the drone.
Robot faces same direction as drone.
Propellers removed.
```
...
local_camera_grid_node
world_posearray_to_json_bridge_node
grid_map_builder_node
map_patch_backend_bridge_node
local_camera_grid_backend_bridge_node
camera_debug_backend_bridge_node
```

Open the presentation page:

```bash
xdg-open http://127.0.0.1:8000/indoor_demo.html
```
...
```text
Mapping Settings → Indoor Static Mission Map
Local Grid Map → Global Mission Grid
```

---

## Terminal 6 — final health check
...
"indoor_static_yaw_zero_rad": <some number>
...
## Terminal 7 — prove the Global Mission Grid is live
