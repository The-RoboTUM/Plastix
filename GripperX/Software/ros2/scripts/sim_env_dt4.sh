...
#  - ROS_DOMAIN_ID=221 instead of the primary twin domain 220: a second parallel
#    twin session takes the next id in the twin band (robot 20-29, twin
#    220-229, PlastiX convention) rather than an unrelated low number, so it
#    does not collide with another twin session on 220 (SR-8 sim domain
#    isolation). Once 220 is verified free (`ROS_DOMAIN_ID=220 ros2 node list`
#    returns nothing), "export ROS_DOMAIN_ID=220" can be set instead.
