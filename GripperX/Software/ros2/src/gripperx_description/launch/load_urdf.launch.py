...
def generate_launch_description():
    ...
    default_gazebo_xacro = PathJoinSubstitution(
        [FindPackageShare("gripperx_description"), "urdf", "gripperx_v1.gazebo.xacro"]
    )
    default_controllers = PathJoinSubstitution(
        [FindPackageShare("gripperx_control"), "config", "ros2_controllers.yaml"]
    )
    ...
    return LaunchDescription(
        [
            ...
            DeclareLaunchArgument(
                "mesh_dir",
                default_value="package://gripperx_description/meshes",
                description="Mesh URI root passed to xacro (file://... for Gazebo).",
            ),
            DeclareLaunchArgument(
                "enable_ros2_control",
                default_value="false",
                description="Include ros2_control block from gripperx_v1.urdf.xacro (real hardware).",
                choices=["true", "True", "false", "False"],
            ),
            DeclareLaunchArgument(
                "ros2_control_name",
                default_value="gripperx_real_system",
                description="ros2_control system name when enable_ros2_control is true.",
            ),
            DeclareLaunchArgument(
                "ros2_control_plugin",
                default_value="gripperx_hardware_interface/GripperXInterface",
