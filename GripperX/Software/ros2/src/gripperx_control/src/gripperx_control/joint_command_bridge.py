...
class JointCommandBridge(Node):
    def __init__(self) -> None:
        ...
        # DT-10: In the sim, sim_steer_bridge takes over publishing the
        # steering commands (real servo steering path via
        # /teleop/direct_steer). This bridge then only publishes the wheel
        # velocities. Default true -> previous behavior unchanged (the real
        # robot doesn't use this bridge anyway; it's purely sim-side).
        self.declare_parameter("publish_steering", True)
        ...
        self.publish_steering = bool(self.get_parameter("publish_steering").value)
    ...
    def command_callback(self, message: JointState) -> None:
        ...
        # DT-10: in the sim, sim_steer_bridge publishes the steering commands
        # (direct_steer override), this bridge then only publishes the wheels.
        if self.publish_steering:
            steering_msg.data = []
