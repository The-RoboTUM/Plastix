...
the real robot none of them currently carries usable data:
  ...
  odom1  /laser/odom            TWO switches, and both must be true for the EKF to
                                ...
                                into the filter). Both default to false.
...
def generate_launch_description():
    ...
    return LaunchDescription(
        [
            ...
            DeclareLaunchArgument(
                "use_mock_sensors",
                # Kept as an argument only so the guard below can give a useful
                # message instead of an unknown-arg error; any true value is
                # rejected.
                default_value="false",
                description=(
            ...
            # With the BNO085 absent, /wheel/odom is the EKF's only other live
            # source when this is off — and /wheel/odom is the topic measured
            # on 2026-08-21 reporting 1 % of the commanded linear velocity and
            # a phantom -0.2594 rad/s yaw.
            #
            ...
            # map_yaml_file defaults to <gripperx_localization>/maps/arena_map.yaml,
            # and THAT FILE DOES NOT EXIST (the maps directory holds
            # testworld_v1_map.* and a README) — so this must default to false,
            # matching the robot's own autostart, which runs live SLAM
            # (gripperx-mapping.sh).
            DeclareLaunchArgument(
