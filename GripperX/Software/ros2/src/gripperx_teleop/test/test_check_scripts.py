...
CHECKS = [
    "check_crab_steering.py",
    "check_key_input.py",
    "check_manoeuvres.py",
    "check_steering_return.py",
    "check_web_ui_assets.py",
]

# EXCLUDED: check_teleop_manoeuvre_path.py -- the only check in this package
# that creates a publisher and puts a Twist on the wire. Whether anything is
# listening is not the point: an automated suite that runs on every build must
# not be a path to commanded motion, and the project's motion-approval rule is
# per test, which a test runner cannot give. Run it by hand, with an approval.
