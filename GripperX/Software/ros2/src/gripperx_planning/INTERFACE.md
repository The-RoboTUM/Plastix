# Contract between `gripperx_localization` and `gripperx_planning`

Packages are split for parallel development. **`gripperx_gazebo` only runs the simulator** (robot + sensors + control); start localization and planning separately.

## `gripperx_gazebo` owns
...
**Launch:** `ros2 launch gripperx_gazebo simulation.launch.py`

## `gripperx_localization` owns
...
**Launch:** `ros2 launch gripperx_localization localization.launch.py`

## `gripperx_planning` owns
...
**Does not use directly:** `/laser/odom` or `ros2_laser_scan_matcher` — laser odometry is started only in `gripperx_localization` (`enable_laser_odometry`) and fused into `/odometry/filtered` by the EKF.

**Launch:** `ros2 launch gripperx_planning navigation.launch.py use_sim_time:=true`
...
# 4 — RViz (optional)
ros2 launch gripperx_localization localization.launch.py  # RViz included by default
```
